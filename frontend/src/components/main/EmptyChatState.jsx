...
import { Sparkles, FileText, ScanEye, Code2, ShieldAlert } from 'lucide-react';

export default function EmptyChatState({ onSelectSuggestion }) {
  const suggestions = [
    {
      icon: <FileText size={16} className="text-neutral-300" />,
      title: 'Analyze Engineering Memo',
      subtitle: 'Audit ASME B31.3 compliance notes'
    },
    {
      icon: <ScanEye size={16} className="text-neutral-300" />,
      title: 'Extract P&ID Blueprints',
      subtitle: 'Read schematic drawings with local OCR'
    },
    {
      icon: <Code2 size={16} className="text-neutral-300" />,
      title: 'Generate Python Script',
      subtitle: 'Parse batch CSV sensor telemetry'
    },
    {
      icon: <ShieldAlert size={16} className="text-neutral-300" />,
      title: 'Air-Gapped Audit',
      subtitle: 'Verify node isolation & zero egress'
    }
  ];
